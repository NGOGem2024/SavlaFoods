...
const InwardOutwardReportScreen = () => {
 ...
 const processApiResponse = (categoryData: CategoryItem[]) => {
   ...
   const uniqueCategories = [...new Set(categoryData.map(item => item.CATDESC))];
 ...
 const fetchCategoriesAndSubcategories = async () => {
   try {
     ...
     try {
       ...
       console.log('API Response data:', JSON.stringify(response).substring(0, 200));
       ...
       if (response && response.output && Array.isArray(response.output)) {
         ...
         console.log('Direct axios response:', JSON.stringify(directResponse.data).substring(0, 200));
         
         if (directResponse.data && directResponse.data.output && Array.isArray(directResponse.data.output)) {
           // Process the response data
       ...
       if (axios.isAxiosError(error)) {
         ...
         Alert.alert('Connection Error', `Failed to connect to server: ${errorMessage}`);
       } else {
 ...
 const handleSearch = async () => {
   ...
   try {
     ...
     console.log('itemCategoryName exact value:', requestData.itemCategoryName);
     console.log('itemSubCategoryName exact value:', requestData.itemSubCategoryName);
     ...
     if (response.data?.data?.length > 0) {
       ...
       if (unit) console.log('- Unit match:', sampleRecord.UNIT_NAME === requestData.unitName);
       if (itemCategory) console.log('- Category match:', sampleRecord.ITEM_CATEG_NAME === requestData.itemCategoryName);
       if (itemSubcategory) console.log('- Subcategory match:', sampleRecord.SUB_CATEGORY_NAME === requestData.itemSubCategoryName);
       console.log('- Customer match:', sampleRecord.CUSTOMER_NAME === requestData.customerName);
     }
     ...
     if (response.data && response.data.success) {
       ...
       const filteredData = response.data.data.filter((item: any) => {
         ...
         const unitMatch = requestData.unitName === null || item.UNIT_NAME === requestData.unitName;
         const categoryMatch = requestData.itemCategoryName === null || item.ITEM_CATEG_NAME === requestData.itemCategoryName;
         const subcategoryMatch = requestData.itemSubCategoryName === null || item.SUB_CATEGORY_NAME === requestData.itemSubCategoryName;
         
         return customerMatch && unitMatch && categoryMatch && subcategoryMatch;
       });
       ...
       Alert.alert('Error', response.data?.message || 'Failed to fetch report data.');
     }
   } catch (error) {
 ...
 apiSubcategories[itemCategory].map((subcategory: string, index: number) => (
 <Picker.Item
 key={index}
 label={subcategory}
 value={subcategory}
 ...
 {showForm ? (
   ...
   <View style={styles.reportContainer}>
     ...
     <View style={[
       styles.reportHeader,
       {backgroundColor: '#f8f8f8', borderColor: isInward ? '#F48221' : '#4682B4'}
     ]}>
       <TouchableOpacity 
         style={[
           styles.backButton,
           {borderColor: isInward ? '#F48221' : '#4682B4', backgroundColor: '#FFFFFF'}
         ]}
         onPress={handleBackToForm}
       >
         <MaterialIcons name="arrow-circle-left" size={20} color={isInward ? '#F48221' : '#4682B4'} />
         <Text style={[styles.backButtonText, {color: isInward ? '#F48221' : '#4682B4'}]}>Back</Text>
       </TouchableOpacity>
       <Text style={[styles.reportTitle, {color: isInward ? '#F48221' : '#4682B4'}]}>
     ...
     <View style={[styles.reportSubHeader, {backgroundColor: '#f8f8f8'}]}>
       {isReportLoading ? (
         <View style={styles.loadingIndicatorContainer}>
           <ActivityIndicator size="small" color={isInward ? '#F48221' : '#4682B4'} />
           <Text style={styles.loadingIndicatorText}>Loading results...</Text>
         </View>
       ) : (
         <View style={styles.resultsSummaryContainer}>
           <MaterialIcons name="receipt-long" size={18} color={isInward ? '#F48221' : '#4682B4'} />
           <Text style={[styles.reportCountText, {color: isInward ? '#F48221' : '#4682B4'}]}>
     ...
     {reportData.length > 0 ? (
       <>
         <View style={[styles.scrollHintContainer, {backgroundColor: '#f8f8f8'}]}>
