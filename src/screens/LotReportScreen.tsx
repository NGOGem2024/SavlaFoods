...
import { FlatList, StyleSheet, Text, TouchableOpacity, View, ActivityIndicator, ScrollView, TextInput } from 'react-native';
...
const LotReportScreen = () => {
  const [selectedTab, setSelectedTab] = useState<'Inwards' | 'Outwards'>('Inwards');
  ...
  useEffect(() => {
    const fetchLotReport = async () => {
      try {
        ...
        const response = await apiClient.get<LotReportResponse>(`${API_ENDPOINTS.GET_LOT_REPORT}/${lotNo}?customerId=${customerId}`);
        
        // Log the API response in a readable format
        console.log('=== LOT REPORT API RESPONSE ===');
        console.log('Success:', response.success);
        console.log('Lot Details:', JSON.stringify(response.lotDetails, null, 2));
        console.log('Summary:', JSON.stringify(response.summary, null, 2));
        console.log('Total Transactions:', response.transactions.length);
        console.log('First Transaction (Sample):', response.transactions.length > 0 ? JSON.stringify(response.transactions[0], null, 2) : 'No transactions');
        console.log('================================');
  ...
  const filteredTransactions = transactions.filter(
    transaction => transaction.TRANSACTION_TYPE === (selectedTab === 'Inwards' ? 'INWARD' : 'OUTWARD')
  );
      ...
      {loading ? (
        ...
        <>
          <View style={styles.detailsContainer}>
            <Text style={styles.lotNo}>Lot No: <Text style={styles.lotNoHighlight}>{lotDetails?.LOT_NO}</Text></Text>
            <Text style={styles.detailText}>Unit Name: <Text style={styles.detailBold}>{lotDetails?.UNIT_NAME}</Text></Text>
            <Text style={styles.detailText}>Item Description: <Text style={styles.detailBold}>{lotDetails?.DESCRIPTION}</Text></Text>
            <Text style={styles.detailText}>Category Name: <Text style={styles.detailBold}>{lotDetails?.CATEGORY_NAME}</Text></Text>
            <Text style={styles.detailText}>Item Mark: <Text style={styles.detailBold}>{lotDetails?.ITEM_MARKS}</Text></Text>
            <Text style={styles.detailText}>Vakkal: <Text style={styles.detailBold}>{lotDetails?.VAKAL_NO}</Text></Text>
            <Text style={styles.detailText}>Available Qty: <Text style={styles.detailBold}>{lotDetails?.AVAILABLE_QTY}</Text></Text>
          </View>

          <View style={styles.tabContainer}>
            <TouchableOpacity
              style={[styles.inwardTabButton, selectedTab === 'Inwards' && styles.activeInwardTab]}
              onPress={() => setSelectedTab('Inwards')}
            >
              <Text style={[styles.tabText, selectedTab === 'Inwards' && styles.activeTabText]}>Inwards</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.outwardTabButton, selectedTab === 'Outwards' && styles.activeOutwardTab]}
                  ...
                  >
                    <View style={{width: '100%'}}>
                      <View style={styles.inwardTableHeader}>
                        ...
                        <Text style={styles.headerCell}>Transaction{'\n'}Date</Text>
                        ...
                        <Text style={styles.headerCell}>Gate Pass{'\n'}Date</Text>
                        <Text style={styles.headerCell}>Running{'\n'}Balance</Text>
                      </View>
                      <FlatList
                        ...
                        renderItem={({ item, index }) => (
                          <View style={[styles.tableRow, index % 2 === 0 ? styles.evenRow : styles.oddRow]}>
                            <Text style={styles.tableCell}>{item.GRN_NO || '-'}</Text>
                            <Text style={styles.tableCell}>{item.TRANSACTION_DATE || '-'}</Text>
                            <Text style={styles.tableCell}>{item.QUANTITY || '-'}</Text>
                            <Text style={styles.tableCell}>{item.ITEM_MARKS || '-'}</Text>
                            <Text style={styles.tableCell}>{item.VAKAL_NO || '-'}</Text>
                            <Text style={styles.tableCell}>{item.BATCH_NO || '-'}</Text>
                            <Text style={styles.tableCell}>{item.EXPIRY_DATE || '-'}</Text>
                            <Text style={styles.tableCell}>{item.UNIT_NAME || '-'}</Text>
                            <Text style={styles.tableCell}>{item.HANDLED_BY || '-'}</Text>
                            <Text style={styles.tableCell}>{item.REMARKS || '-'}</Text>
                            <Text style={styles.tableCell}>{item.GATEPASS_NO || '-'}</Text>
                            <Text style={styles.tableCell}>{item.GATE_PASS_DATE || '-'}</Text>
                            <Text style={styles.tableCell}>{item.RUNNING_BALANCE || '-'}</Text>
                          </View>
                        )}
                        style={styles.flatList}
                        ListEmptyComponent={() => (
                          <View style={styles.emptyContainer}>
                            <Text style={styles.emptyText}>No Inwards transactions found</Text>
                          </View>
                        )}
                      />
                    </View>
                  ...
                  >
                    <View style={{width: '100%'}}>
                      <View style={styles.outwardTableHeader}>
                        <Text style={[styles.headerCell, {width: 110}]}>Delivery{'\n'}Challan No</Text>
                        <Text style={styles.headerCell}>Transaction{'\n'}Date</Text>
                        <Text style={styles.headerCell}>Quantity</Text>
                        <Text style={styles.headerCell}>Item Marks</Text>
                        <Text style={styles.headerCell}>Vakal No</Text>
                      ...
                      <FlatList
                        ...
                        renderItem={({ item, index }) => (
                          <View style={[styles.tableRow, index % 2 === 0 ? styles.evenRow : styles.oddRow]}>
                            <Text style={[styles.tableCell, {width: 110}]}>{item.DELIVERY_CHALLAN_NO || '-'}</Text>
                            <Text style={styles.tableCell}>{item.TRANSACTION_DATE || '-'}</Text>
                            <Text style={styles.tableCell}>{item.QUANTITY || '-'}</Text>
                            <Text style={styles.tableCell}>{item.ITEM_MARKS || '-'}</Text>
                            <Text style={styles.tableCell}>{item.VAKAL_NO || '-'}</Text>
                        ...
                        ListEmptyComponent={() => (
                          <View style={styles.emptyContainer}>
                            <Text style={styles.emptyText}>No Outwards transactions found</Text>
                          </View>
                        )}
                      />
                    </View>
