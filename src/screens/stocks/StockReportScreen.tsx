...
const StockReportScreen: React.FC = () => {
 ...
 const logAndSetItemSubCategory = (value: string) => {
   console.log('Item Sub Category changed:', value);
   setItemSubCategory(value);
   
   // Find the matching subcategory and store its description
   if (value) {
     const selectedCategory = subCategories.find(cat => cat.SUBCATID.toString() === value);
     if (selectedCategory) {
       const subcatDesc = selectedCategory.SUBCATDESC;
       console.log('Selected subcategory description:', subcatDesc);
       setItemSubCategoryText(subcatDesc);
     } else {
 ...
 console.log('Item Sub Category (ID):', itemSubCategory);
 ...
 const onToDateChange = (event: DateTimePickerEvent, selectedDate?: Date) => {
   ...
   if (selectedDate) {
     console.log(
       `To date changing from ${toDate?.toISOString() || 'null'} to ${selectedDate.toISOString()}`,
     );
 ...
 onSelect={logAndSetItemSubCategory}
 placeholder="--SELECT--"
 />
 )}
 </View>
 ...
 <TouchableOpacity
   ...
   <Text style={isDateSelected ? styles.dateText : styles.placeholderText}>
