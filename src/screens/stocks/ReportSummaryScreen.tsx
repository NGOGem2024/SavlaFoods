...
const ReportSummaryScreen: React.FC = () => {
 ...
 const [summaryData, setSummaryData] = useState<SummaryData | ItemWiseData[] | null>(null);
 ...
 const [lastApiRequestTime, setLastApiRequestTime] = useState<Date | null>(null);
 ...
 const onFromDateChange = (
   ...
   if (selectedDate) {
     console.log(`From date changing from ${fromDate.toISOString()} to ${selectedDate.toISOString()}`);
     console.log(`API format: ${formatApiDate(fromDate)} to ${formatApiDate(selectedDate)}`);
 ...
 const onToDateChange = (event: DateTimePickerEvent, selectedDate?: Date) => {
   ...
   if (selectedDate) {
     console.log(`To date changing from ${toDate.toISOString()} to ${selectedDate.toISOString()}`);
     console.log(`API format: ${formatApiDate(toDate)} to ${formatApiDate(selectedDate)}`);
 ...
 const handleApplyDates = async () => {
   ...
   console.log(`[${requestId}] Apply Date Range button pressed for report type: ${reportType}`);
   ...
   try {
     ...
     console.log(`[${requestId}] Using dates: From=${currentFromDate.toISOString()}, To=${currentToDate.toISOString()}`);
     ...
     console.log(`[${requestId}] Formatted dates: From=${fromDateStr}, To=${toDateStr}`);
     ...
     const apiEndpoint = reportType === 'all' 
       ? API_ENDPOINTS.GET_ALL_SUMMARY 
       : API_ENDPOINTS.GET_ITEMWISE_SUMMARY;
     
     console.log(`[${requestId}] Using API endpoint: ${apiEndpoint} for report type: ${reportType}`);
     ...
     if (response.data && response.data.success) {
       ...
       if (reportType === 'itemwise' && Array.isArray(response.data.data)) {
         console.log(`[${requestId}] Received ${response.data.data.length} items from API`);
         console.log(`[${requestId}] Full item-wise data:`);
         response.data.data.forEach((item, index) => {
           console.log(`[${requestId}] Item ${index + 1}:`, JSON.stringify(item, null, 2));
         });
       } else {
         console.log(`[${requestId}] API response data received:`, 
           reportType === 'all' 
             ? JSON.stringify(response.data.data, null, 2)
             : `Array with ${Array.isArray(response.data.data) ? response.data.data.length : 0} items`);
       }
       ...
       const errorMessage = response.data?.message || 'Failed to fetch report data';
 ...
 const isSummaryData = (data: any): data is SummaryData => {
   return data && !Array.isArray(data) && 'inward' in data && 'outward' in data;
 };

 const isItemWiseData = (data: any): data is ItemWiseData[] => {
   return data && Array.isArray(data) && data.length > 0 && 'ITEM_NAME' in data[0];
 };
 ...
 const renderSummaryData = () => {
   ...
   console.log('Outward data for rendering:', JSON.stringify(outward, null, 2)); 
   console.log('Summary data for rendering:', JSON.stringify(summary, null, 2));
   
   // Show full raw data in logs for troubleshooting
   console.log('TOTAL_INWARD_QUANTITY type:', typeof inward.TOTAL_INWARD_QUANTITY);
   console.log('TOTAL_INWARD_QUANTITY raw value:', inward.TOTAL_INWARD_QUANTITY);
   console.log('TOTAL_INWARD_QUANTITY formatted:', formatNumber(inward.TOTAL_INWARD_QUANTITY));
   console.log('TOTAL_OUTWARD_QUANTITY raw value:', outward.TOTAL_OUTWARD_QUANTITY);
   console.log('TOTAL_OUTWARD_QUANTITY formatted:', formatNumber(outward.TOTAL_OUTWARD_QUANTITY));
   console.log('TOTAL_QUANTITY_DIFFERENCE raw value:', summary.TOTAL_QUANTITY_DIFFERENCE);
   console.log('TOTAL_QUANTITY_DIFFERENCE formatted:', formatNumber(summary.TOTAL_QUANTITY_DIFFERENCE));
   ...
   if (!hasData) {
     return (
       <View style={styles.reportSection}>
         <Text style={styles.emptyMessage}>
           No report data found for the selected date range. Try selecting a different date range.
         </Text>
       </View>
     );
   }
 ...
 const renderItemWiseData = () => {
   if (!summaryData) {
     ...
     return (
       <View style={styles.reportSection}>
         <Text style={styles.emptyMessage}>
           Select a date range and press "Apply Date Range" to view item-wise data
         </Text>
       </View>
     );
   }
   ...
   if (!isItemWiseData(summaryData)) {
     ...
     return (
       <View style={styles.reportSection}>
         <Text style={styles.emptyMessage}>
           No item-wise data available. Please select the "Item-wise" report type.
         </Text>
       </View>
     );
   }
   ...
   if (itemWiseData.length === 0) {
     return (
       <View style={styles.reportSection}>
         <Text style={styles.emptyMessage}>
           No item data found for the selected date range. Try selecting a different date range.
         </Text>
       </View>
     );
   }
   ...
   return (
     <View style={styles.reportSection}>
       <Text style={styles.sectionTitle}>Item-wise Summary ({itemWiseData.length} items)</Text>
 ...
 <TouchableOpacity style={styles.retryButton} onPress={handleApplyDates}>
