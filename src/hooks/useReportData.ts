  ...
  const fetchReport = async (filters: ReportFilters): Promise<boolean> => {
    ...
    try {
      ...
      const requestData = {
        ...
        itemCategoryName: filters.itemCategory ? filters.itemCategory.trim() : null,
        itemSubCategoryName: filters.itemSubcategory ? filters.itemSubcategory.trim() : null,
        unitName: filters.unit ? filters.unit.trim() : null,
      };

      console.log('==== REQUEST DATA DETAILS ====');
      console.log('URL:', isInward ? API_ENDPOINTS.GET_INWARD_REPORT : API_ENDPOINTS.GET_OUTWARD_REPORT);
