...
export const usePdfGeneration = ({ isInward }: UsePdfGenerationProps) => {
  ...
  // Generate PDF report
  const generatePdf = async (reportData: any[], fromDate: Date, toDate: Date, customerName: string, filters: any) => {
    try {
      ...
      const sanitizedReportData = reportData.map(item => {
        ...
        // Process each string property to replace problematic characters
        Object.keys(sanitizedItem).forEach(key => {
          if (typeof sanitizedItem[key] === 'string') {
            sanitizedItem[key] = sanitizedItem[key]
              ...
              .replace(/[\u2013\u2014\u2015]/g, '-'); // Replace various dash characters with hyphen
          }
        });
      ...
      if (Platform.OS === 'ios') {
        ...
        if (!hasPermission) {
          console.log('Storage permission denied, using app-specific directory as fallback');
      ...
      let fontSize = totalRows <= 10 ? 9 : (totalRows <= 20 ? 8 : (totalRows <= 40 ? 7 : 6));
      ...
      for (let pageNumber = 0; pageNumber < totalPages; pageNumber++) {
        ...
        const endRow = Math.min(startRow + rowsOnThisPage, sanitizedReportData.length);
        ...
        if (isFirstPage) {
          ...
          if (filters.unit) filtersText += `, Unit: ${filters.unit}`;
          if (filters.itemCategory) filtersText += `, Category: ${filters.itemCategory}`;
          if (filters.itemSubcategory) filtersText += `, Subcategory: ${filters.itemSubcategory}`;
        ...
        for (let colIndex = 0; colIndex < tableColumns.length; colIndex++) {
          ...
          let columnAlign: 'left' | 'center' | 'right' = colIndex === 0 || colIndex === 11 ? 'center' : 'left';
        ...
        for (let i = startRow; i < endRow; i++) {
          ...
          // Render additional fields (columns 4-13)
          // ...rest of the columns with similar patterns
          // For brevity, I've omitted rendering each individual column
          
          // Draw remaining columns (simplified for brevity)
      ...
      try {
        ...
        if (Platform.OS === 'android') {
          try {
            await RNBlobUtil.fs.scanFile([{ path: publicFilePath, mime: 'application/pdf' }]);
        ...
        Alert.alert('Error', 'Failed to save PDF file: ' + (error instanceof Error ? error.message : String(error)));
        throw error;
      }
    } catch (error) {
      console.error('Error generating PDF:', error);
      Alert.alert('Error', 'Failed to generate PDF: ' + (error instanceof Error ? error.message : String(error)));
      throw error;
    } finally {
