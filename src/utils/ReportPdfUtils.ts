...
export const drawWrappedText = (
  ...
  const font = options?.font || page.doc.embedStandardFont(StandardFonts.TimesRoman);
 ...
 */
export const getUniqueFileName = async (basePath: string, baseFileName: string): Promise<string> => {
...
export const requestStoragePermission = async (): Promise<boolean> => {
  ...
  try {
    ...
    if (androidVersion >= 33) {
      ...
      const mediaImagesGranted = granted[PermissionsAndroid.PERMISSIONS.READ_MEDIA_IMAGES] === 
 ...
 */
export const showDownloadNotification = (filePath: string, fileName: string, isInward: boolean) => {
  ...
  if (Platform.OS === 'android') {
    ...
    PushNotification.createChannel(
      {
        channelId,
        channelName: isInward ? 'Inward PDF Downloads' : 'Outward PDF Downloads',
        channelDescription: `Notifications for ${isInward ? 'Inward' : 'Outward'} PDF downloads`,
      ...
      (created: boolean) => {
        console.log(`Notification channel created status: ${created ? 'success' : 'failed'}`);
        if (!created) {
          console.log('Failed to create notification channel, using Toast as fallback');
          ToastAndroid.show('PDF downloaded to Downloads folder', ToastAndroid.LONG);
        }
      }
    );
